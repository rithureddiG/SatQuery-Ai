...
class StubModelAdapter:
    ...
    def __init__(self, name: str, task: str, description: str, capabilities: List[str],
                 vram_estimate_mb: int, phase_target: str = "Phase 1"):
        self.name = name
        self.task = task
    ...
    def health(self) -> Dict[str, Any]:
        return {
            "name": self.name, "task": self.task, "status": self.status,
            "installed": False, "checkpoint_verified": False,
            "vram_estimate_mb": self.vram_estimate_mb,
            "message": f"Model '{self.name}' is unavailable; scheduled for {self.phase_target}.",
        }
...
class ModelRegistry:
    ...
    def _register_default_models(self) -> None:
        ...
        for key, name, task, capabilities, vram in planned:
            self.register(key, StubModelAdapter(name, task, f"{name} runtime", capabilities, vram))

    def register(self, key: str, adapter: ModelAdapter) -> None:
        if key in self._ALIASES:
            raise ValueError(f"Register canonical key instead of alias '{key}'")
        self._models[key] = adapter

    def get(self, key: str) -> Optional[ModelAdapter]:
        return self._models.get(self._ALIASES.get(key, key))
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        for key, model in self._models.items():
            ...
            result.append({
                ...
                "key": key,
                "entity_type": "MODEL",
                "name": getattr(model, "name", key),
                "task": getattr(model, "task", "unknown"),
                "status": model.status,
                "runtime_status": model.status,
                "installation_status": health.get("installed", False),
                "checkpoint_status": "VERIFIED" if health.get("checkpoint_verified") else "UNVERIFIED",
                "checkpoint_path": health.get("checkpoint_path"),
                "checkpoint_sha256": health.get("checkpoint_sha256"),
                "source_uri": health.get("source_uri"),
                "dataset_id": health.get("dataset_id"),
                "training_status": health.get("training_status"),
                "experiment_id": health.get("experiment_id"),
                "code_commit": health.get("code_commit"),
                "config_hash": health.get("config_hash"),
                "evaluation_status": health.get("evaluation_status"),
                "evaluation_metrics": health.get("evaluation_metrics"),
                "fallback_mode": health.get("fallback_mode", model.status == "FALLBACK"),
                "device": health.get("device"),
                "precision": health.get("precision"),
                "memory_requirement": getattr(model, "vram_estimate_mb", 0),
                "last_healthcheck": health.get("last_healthcheck"),
